...
exports.signin = async (req, res, next) => {
  try {
    ...
    if (user) {
      ...
      await Models.User.updateOne({ _id: user._id }, { token: token });
      ...
      return res.status(200).json(sendResponse(response, 200, RECORDS_FOUND));
    }
  } catch (error) {
    console.log(error);
    // logger.error("Whooops! This broke with error: ", error);
...
exports.reqestResetPasswords = async (req, res, next) => {
  try {
    ...
    const userExists = await Models.User.findOne({ email: req.body.email });
